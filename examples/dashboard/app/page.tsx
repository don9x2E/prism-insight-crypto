...
const VALID_TABS: TabType[] = ["dashboard", "ai-decisions", "trading", "watchlist", "insights", "jeoningu-lab", "crypto-benchmark"]
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 404) {
            setDataError(language === "ko"
              ? `${market === "US" ? "미국" : "한국"} 시장 데이터가 없습니다. JSON 생성 스크립트를 먼저 실행해 주세요.`
              : `${market === "US" ? "US" : "KR"} market data is missing. Run dashboard JSON generator first.`
            )
            setData(null)
            return
          }
        ...
        setDataError(language === "ko"
          ...
          : `Failed to load ${market === "US" ? "US" : "KR"} market data.`
        )
      }
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-6 max-w-[1600px]">
        ...
        {activeTab === "crypto-benchmark" && <CryptoBenchmarkPage />}
      </main>
